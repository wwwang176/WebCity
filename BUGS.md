...
- **狀態**: 待修
- **修法會動平衡**: 接上活的載重率之後，這條路線的載重是 308 —— 遠超過拒載門檻 1.5，
  公車會**當場對所有人關閉**，搭乘人數掉到零。見下面的刻度問題。

### BUG-344: 一班公車跑完一圈要六個遊戲日
...
- **後果**: 任何一條公車路線只要一天超過約 9 人次就爆表。運能模型本身沒有錯
  ...
  公車一天跑不完一圈。
- **這是平衡問題不是程式問題**。可以動的旋鈕:車速、`ticksPerDay`、每台車的座位數，
  或是讓運能不要跟「一天跑幾圈」綁在一起。
