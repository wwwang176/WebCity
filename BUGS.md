...
- **症狀**: BUG-363 把負載分攤從歐氏直線改成覆蓋的擁有者之後，**每一格的需求全部
  記在「沿馬路走過來最便宜的那一座」頭上**。第二近的設施明明也涵蓋得到這一區，
  卻永遠掛著 0。玩家的存檔實測:hospital_3 顯示 4034 / 1750（230%），
  而 hospital_1 是 **0 / 1750**。
- **根因**: 覆蓋的擁有者圖每格只記**一座**（`GridCoverageArray.owners` 是單一索引）。
  合併之後就丟掉了逐設施的距離圖，所以答得出「最近的是誰」，答不出「還有誰
  涵蓋得到」—— 而後者正是溢出需要的。
- **遊戲裡本來就有做對的例子**: 垃圾車與靈車（`GlobalCoverageService.collectPending`）
  一直是這樣挑的 ——
  ...
  三件事:**只看涵蓋得到那一格的**、**滿了就換下一座**、**在還有空位的裡面挑最近的**。
- **修法**:
  1. `RoadCoverageMap` 留下逐設施的涵蓋圖（本來就算出來了，只是合併完就丟）。
     `getCoveringIndices(x, y)` 回「涵蓋得到這一格的每一座，由近到遠」。
  2. `SpilloverLoadDistributor` —— 近的優先，滿了換下一座。需求依「離自己最近的
     那座有多近」排隊處理:緊鄰醫院的街區先占走它，遠一點的才溢到第二近的。
     反過來的話，一個邊陲街區可以先把市中心的醫院占滿。順序是**決定性的**
     （同成本比 id），因為這份攤派每 6 tick 重算，亂數會讓面板數字自己跳動。
  3. 全部滿了時，剩下的量記在**最近的那一座**。硬截在容量的話，沒有任何一座會
     超過 100% —— 而「超載」正是這些數字的用途。
  4. `getServingFacilityId` / `getLoadRatioAt` 跟著同一條規則:**還有空位的裡面
     最近的那一座**。所以最近那間滿了、第二間還很空時，這一格是被照顧到的。
  5. 警察、消防、醫療、學校四個都改;垃圾與殯葬本來就是這樣收運的。
     `GlobalCoverageService` 的 `mergedOwnerMap` 拿掉，改用它本來就有的逐設施距離圖。
- **順帶修正一個我先前的誤判**: BUG-363 那一輪我說「hospital_1 覆蓋 53 商業 + 59 工業、
  住宅 0 棟，服務不到任何人」。那是拿**擁有者**（最近的那一座）算的 —— 正確的說法是
  「hospital_1 不是任何住宅區**最近**的那一座」，但它**涵蓋得到**其中很多。
  換成溢出之後它收了滿滿 1750。
- **實測**（玩家的 12,408 人存檔）:

  | | 歐氏（原本） | 只認最近（BUG-363） | 溢出（現在） |
  |---|---|---|---|
  | hospital_1 | 844 / 1750 | **0** / 1750 | **1750 / 1750** |
  | hospital_3 | 2954 / 1750 | 4034 / 1750（230%） | 2288 / 1750（131%） |
  | 那棟住宅的面板 | 黃字 over capacity | 紅字 **overloaded** | 黃字 over capacity |

- **嚴重性**: 高（第二座設施等於白蓋）
- **狀態**: 已修復

### BUG-364: 學校爆滿在建築面板上完全看不出來 ✅ 已修復

- **症狀**: 建築面板的警告清單對垃圾、醫院、警察、消防都有「over capacity /
  overloaded」兩級警告，**唯獨教育沒有**。玩家的存檔實測 High School 收了
  5,872 個學生而容量 500（超收 11 倍），點開任何一棟住宅都看不到這件事 ——
  Education 的圓點還是綠的，警告清單裡也沒有這一條。
- **根因**: `Game` 往 `SelectedBuilding` 塞了 `garbageLoadRatio`、`hospitalLoadRatio`、
  `policeLoadRatio`、`fireLoadRatio`，就是沒有 education 的。`BuildingPanel` 自然也
  沒有那一段。
- **修法**: `Game` 補上 `educationLoadRatio`（全城最滿的那一種學校），面板照其他
  四個服務的樣式加兩級警告。
- **實測**: 玩家的存檔 —— 高中需求 6,003 / 容量 500，面板現在紅字寫著
  「Schools overcrowded」。
- **嚴重性**: 中
- **狀態**: 已修復

### BUG-363: 遊戲裡有兩套互不相容的「哪一間設施服務我」 ✅ 已修復

- **症狀**: 覆蓋（圓點、圖層、`getCostRatio`）算的是**沿馬路走過來的成本**;
  負載分配（`distributeLoadToNearest`）算的是**歐氏直線距離**。兩者會給出不同的答案。
- **具體會怎樣**: 河對岸一間直線很近、開車要繞一大圈的醫院，會吸走這一區的病人
  （負載記在它頭上），但它的道路覆蓋根本到不了這裡。於是那間醫院顯示爆量、
  實際上服務不到任何人，而真正服務這一區的醫院顯示很空。
- **修法**: `distributeLoadToNearest` 改名為 `distributeLoadToServingFacility`，
  必須帶一支 `ownerOf(x, y)` —— 也就是覆蓋洪水判定「用最低成本涵蓋這一格」的那一座。
  警察、消防、醫療、學校四個全部改走它（醫療與學校原本各自寫了一份內嵌的歐氏迴圈）。
  **順帶解掉 BUG-100 同一類的問題**:覆蓋只從運作中的設施淹出去，所以停電的學校
  再也吸不走學生 —— 舊的 `SchoolService.findNearest` 連沒電的都算在內。
- **沒服務到的需求不會消失**: 需求點在上一次覆蓋重算之後失去覆蓋時，它仍然算進
  `loadRatio` 的分子（`unassigned`）。歸零的話，城市會在崩潰的當下顯示得更健康。
- **垃圾與殯葬本來就是對的**: `GlobalCoverageService.collectPending` 一直是走
  `mergedDistanceMap`（道路成本）。大眾運輸也不需要改 —— 乘載是
  `stop.dailyRiders`，市民實際尋路後的上車紀錄，不是就近攤派。
- **前置條件**: 負載現在需要覆蓋先算過。四條路徑都確認會重算 ——
  載入存檔（`recalculateAllRoadCoverage` 在載入階段）、蓋／拆設施
  （`recalculateServiceCoverage`）、道路變動（`roadCoverageDirty`，每 6 tick 批次）、
  斷電復電（`updateRoadServiceOps` 立即重算）。
- **實測**（玩家的 12,408 人存檔）: 這個改動揭露了一件舊規則藏起來的事 ——
  **hospital_1 蓋在商業／工業區，覆蓋 53 棟商業 + 59 棟工業，住宅 0 棟。它服務不到
  任何人。** 歐氏規則攤了 844 的需求給它，看起來半忙;實際上全城 12,363 人**全部**
  由 hospital_3 承擔，4034 / 1750 = **2.30 倍**（≥2.0 等於完全沒有醫院）。
- **嚴重性**: 中
- **狀態**: 已修復

### BUG-362: 醫院爆量時，住宅的服務圓點與 Health 圖層照樣是綠的 ✅ 已修復

- **症狀**: 城裡的醫院已經爆到 200%，點開緊鄰它的住宅，Health 圓點仍然是**最綠的**;
  打開 Health 圖層，那些建築仍然是**綠色**。其他四個服務（警察、消防、垃圾、教育）
  一樣。
- **根因**: 圓點的顏色來自 `buildServiceStatus()` → `service.getCostRatio(x, y)`，
  圖層的建築顏色來自 `getRoadCostOverlay()` → `cost / budget`。**兩者都只是
  「沿馬路從最近一座設施走過來的成本」—— 純粹是距離。** 容量與負載從來不是輸入:
  `recalculateCoverage()` 只會過濾掉沒水沒電、沒接到路的設施。
- **而爆量是真的有後果的**: `loadRatioToDeathMultiplier()` —— 負載 ≤1.0 時死亡率
  ×0.3（醫院全效），≥2.0 時 ×1.0（**等於完全沒有醫院**），中間線性。所以一間爆到
  200% 的醫院對死亡率的貢獻歸零，而圓點還是滿綠。
- **面板不是完全沒講，但講在別的地方**: 同一個面板下方的警告清單有
  `Hospital over capacity` / `Hospital overloaded`。所以實際情況是**上面綠燈、
  下面紅字**，而玩家先看到的是那排燈。
- **而且那條警告是全城的**: `getLoadRatio()` 是全城總需求 ÷ 全城總容量。城市另一頭的
  醫院爆量，你這棟樓也會跳警告;你隔壁那間爆量而全城平均還好，反而不會跳。
  **覆蓋是逐格的，負載是全城的，兩者從來沒有對在一起**（見 BUG-363）。
  **這一半後來才補**:使用者回報「住宅旁邊就是國小、那間沒爆量，面板卻寫教育爆量」——
  就是這條。五個警告（醫院、警察、消防、垃圾、教育）現在全部改吃
  `serviceLoadRatiosAt(state, x, y)`。順帶修掉一個既有的分家:重新整理選取的那一段
  只塞了三欄，police/fire 停在剛點下去那一刻的值。
- **修法**（本輪）: 讓覆蓋洪水記下每一格是被**哪一座**設施用最低成本涵蓋的，
  於是逐格答得出「服務我的那間設施現在多滿」。圓點與圖層改成取距離與負載
  **兩者中比較糟的那一個**。
- **修法**:
  1. `GridCoverageArray` 多一條 `Uint16Array`，記下每一格是被**哪一座**設施用最低
     成本涵蓋的。成本被改寫時擁有者跟著改寫 —— 兩者一起動，才不會出現「成本是 A 的、
     擁有者是 B」。`GlobalCoverageService`（垃圾、殯葬）有自己的距離圖，另外記一份
     `mergedOwnerMap`。
  2. `RoadCoverageService.getServingFacilityId(x, y)` / `getLoadRatioAt(x, y)` ——
     逐格答得出「服務我的那一座現在多滿」。子類別各自回報單一設施的負載與容量;
     學校用的是**想讀的人數**而不是在學人數（後者頂多等於容量，超收十倍看起來會
     剛剛好）。
  3. `serviceSeverity(cost, load)` = 兩者取**比較糟**的一個。負載 1.0 算 0、
     2.0 算 1 —— 這條線跟 `loadRatioToDeathMultiplier` 是同一組端點。
  4. 圓點（`serviceDot.ts`，從 TSX 抽出來才測得到）與圖層的建築上色都改吃嚴重度。
     滑過圓點看得到是距離還是負載在扣分。
  5. `read.coverage()` 一併帶出 `load`、`severity`、`facilityId`。
- **實測**（玩家的 12,408 人存檔，兩間醫院一空一爆）:

  | | 醫院 1（844/1750） | 醫院 3（2954/1750） |
  |---|---|---|
  | 隔壁那一格的距離 | 0 | 0 |
  | 負載 | 0.48 | **1.69** |
  | 修前的階數 | 0（綠） | **0（綠）** |
  ...
  953 格覆蓋中 **350 格變色，全部往壞的方向**（嚴重度取最大值，不可能變好）。
  隨手點一棟住宅:education 距離 0 而負載 **11.86 倍** → 圓點從最綠變最紅。
- **嚴重性**: 高（玩家依賴的第一眼訊號，在最需要示警的時候是綠的）
- **狀態**: 已修復
...
- **症狀**: agent 載入存檔之後，HUD 顯示的是**上一局**的人口與市庫，而地圖畫的是
  新的城市 —— 數字跟畫面對不起來。
- **根因**: `createGameUI()` 每次做一個新的 `#game-ui` 掛到 body 上，**沒有人拆舊的**。
  `document.querySelectorAll('#game-ui').length === 2`，上面那份是舊的。
  而且 Solid `render()` 回傳的**解除函式被丟掉了** —— 舊 root 的訂閱與 effect
  一直活著，不只是 DOM 殘留。
- **跟 BUG-351 同一類**: `startGame` 沒有清掉上一個畫面的東西。上一輪補了主選單，
  漏了遊戲 UI 自己。
- **修法**: `createGameUI()` 自己收拾自己的前一份（dispose + remove）。放在它裡面
  而不是呼叫端 —— 呼叫端每多一個就多一次「忘記拆」的機會，而忘記的徵兆是兩份 UI
  疊著，不是錯誤訊息。
- **怎麼發現的**: 拿玩家的 12,410 人存檔驗證覆蓋圖層時，HUD 寫著「人口 0」。
  那是前一局測試遊戲的數字。**又是靠看畫面發現的，不是靠 API。**
- **嚴重性**: 高（換局之後所有 HUD 數字都是錯的）
- **狀態**: 已修復
