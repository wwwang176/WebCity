...
- **修法**:
  - `beginHousingRelocation` 多收一個 `isHousingValid(pos)`，提交搬遷前問一次現在的格子。
  - 新增 `refreshOccupancy(counts)`;`SimulationLoop` 每次重建位置索引時順手換上新的一份，
    落後從 50 個 tick 降到 6 個（與慢速槽同一級）。
  - `Citizen.removed` 墓碑:`CitizenManager` 移除時在物件上留記號，切片器據此跳過。
- **同一種病的鄰居（未修）**: `JobRelocationSlicer`（BUG-109）有一模一樣的形狀 —— 名單一
  拍就是上百個 tick，而它只擋 `homeId === null`，不擋墓碑、不檢查候選工作地是否還在。
  墓碑已經立好了，接上去是一行。記在 TODO。
- **一併記下（改不掉的）**: 一輪橫跨 50 個 tick，決策因此混了兩個時間點 —— `scoreHousing`
  讀的是市民**當下**的學歷與工作地，配的卻是**開輪時**的住宅環境。而這 50 個 tick 裡新長
  出來的住宅不在候選名單裡，完全不會被考慮。這是攤開本身的代價。
- **嚴重性**: 高（前兩項會產生不存在的住址與超額入住）
- **狀態**: 已修復
