...
### BUG-363: 遊戲裡有兩套互不相容的「哪一間設施服務我」 ⬜ 未修復
...
- **為什麼不順手修**: 把負載改成走覆蓋的擁有者，全城的負載數字會重新分佈 ——
  死亡率、犯罪抑制、火災損害都吃 `getLoadRatio()`。那是平衡調整，不是筆誤修正。
- **本輪的處理**: 逐格的「服務我的那間設施」採用**道路成本**的答案（那才是覆蓋的
  定義）。所以面板顯示的是「真正服務你的那間設施的負載」，而那個負載本身的分攤
  仍然是歐氏的。這比現況好，但兩套規則還在。
- **嚴重性**: 中
- **狀態**: 未修復
