...
### BUG-330: 七萬人時每 1.5 秒卡一次 —— 快樂度逐市民重算，同一棟房子查幾十遍
...
- **可能有用的方向**（未實作、未量測）: 切片，跟 `advanceCommuteFill` 同一招（BUG-329）。
  快樂度本來就每 6 個 tick 才更新一次，切成 N 份的話每位市民變成每 6N 個 tick 更新。
  **這是行為改變不是純贏**:快樂度反應變慢，而它會影響遷入遷出與建築廢棄。切幾份要看
  快樂度實際變化得多快 —— 還沒量。
