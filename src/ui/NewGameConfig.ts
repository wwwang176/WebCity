...
/**
 * Create the New Game configuration screen.
 * Vanilla HTML+CSS — follows the same glass-morphism style as MainMenu.ts.
 */
export function createNewGameConfig(
  ...
  el.innerHTML = `
    <style>
      ...
      .ngc-panel {
        ...
        animation: ngc-slide 0.25s ease;
      }
      @keyframes ngc-slide {
      ...
      .ngc-close:hover { background: rgba(239,83,80,0.2); color: #ef5350; }
      .ngc-body {
        display: flex; gap: 24px; padding: 20px 24px;
      }
      .ngc-left { flex: 1; display: flex; flex-direction: column; gap: 16px; }
      .ngc-right {
      ...
      .ngc-seed-label {
        font-size: 11px; color: rgba(144,202,249,0.5);
      ...
      .ngc-seed-input {
        ...
        border-radius: 6px; color: #d0d8e8;
        padding: 6px 10px; font-size: 13px;
        width: 120px; font-family: inherit;
        font-variant-numeric: tabular-nums;
      }
      .ngc-seed-input:focus {
        outline: none; border-color: rgba(66,165,245,0.5);
      }
      .ngc-dice-btn {
        background: rgba(15,25,50,0.7);
        border: 1px solid rgba(66,165,245,0.2);
      ...
      .ngc-btn-group button {
        ...
        color: #8899b0; padding: 6px 14px;
        font-size: 12px; font-weight: 500; cursor: pointer;
        transition: all 0.12s; font-family: inherit;
      }
      .ngc-btn-group button:first-child { border-radius: 6px 0 0 6px; }
      .ngc-btn-group button:last-child { border-radius: 0 6px 6px 0; }
      ...
      .ngc-toggle::after {
        ...
        border-radius: 50%; background: #8899b0;
        transition: all 0.2s;
      }
      .ngc-toggle.on::after {
        left: 21px; background: #42a5f5;
      }

      .ngc-freq-row {
        display: flex; align-items: center; gap: 8px;
        margin-top: 4px; padding-left: 100px;
    ...
    <div class="ngc-panel">
      <div class="ngc-header">
        <div class="ngc-title">New Game</div>
        <button class="ngc-close" id="ngc-close">&times;</button>
      </div>
      <div class="ngc-body">
        <div class="ngc-left">
          <!-- Seed -->
          ...
          <!-- Mountain -->
          <div class="ngc-row">
            <div class="ngc-label">Mountains</div>
            <div class="ngc-row-body">
              <input type="range" class="ngc-slider" id="ngc-mountain" min="0" max="2" step="1" value="1">
              <span class="ngc-slider-val" id="ngc-mountain-val">Medium</span>
            </div>
          </div>
          <!-- Starting Funds -->
          <div class="ngc-row">
        ...
        <div class="ngc-right">
          <canvas class="ngc-preview-canvas" id="ngc-preview" width="240" height="240"></canvas>
  ...
  const mountainSlider = el.querySelector('#ngc-mountain') as HTMLInputElement;
  const mountainVal = el.querySelector('#ngc-mountain-val') as HTMLSpanElement;
  ...
  const MOUNTAIN_LABELS = ['Low', 'Medium', 'High'] as const;
  const MOUNTAIN_VALUES = ['low', 'medium', 'high'] as const;
  ...
  mountainSlider.addEventListener('input', () => {
    const i = parseInt(mountainSlider.value, 10);
    mountainVal.textContent = MOUNTAIN_LABELS[i];
    config.mountainAmount = MOUNTAIN_VALUES[i];
    updatePreview();
  });
  ...
  el.querySelector('#ngc-close')!.addEventListener('click', onBack);
