...
export function buildCarGeometry(): THREE.BufferGeometry {
  // Lower body (chassis + hood + trunk) — 1.4× original height
  const body = new THREE.BoxGeometry(0.22, 0.056, 0.09);
  body.translate(0, 0.028, 0);
  setVertexColors(body, 1, 1, 1);

  // Cabin with windows (set back slightly) — original size
  const cabin = new THREE.BoxGeometry(0.1, 0.033, 0.082);
  cabin.translate(-0.02, 0.069, 0);
  setVertexColors(cabin, 0.1, 0.13, 0.18);

  // Wheels (4 corners)
  const parts: THREE.BufferGeometry[] = [body, cabin];
  const wheelGeo = new THREE.BoxGeometry(0.03, 0.023, 0.014);
