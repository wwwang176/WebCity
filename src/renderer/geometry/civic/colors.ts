...
export const CIVIC_COLORS: Partial<Record<InfraType, CivicColor>> = {
   ...
   * 抽水廠取**河的顏色**。
   *
   * 使用者：「抽水站的主顏色參考河流的顏色」（在此之前試過綠色系）。
   * 色相直接對齊地形的水面（`TERRAIN_COLORS[WATER]` = 0x2196f3），明度壓一階
   * —— 原色是水面在陽光下的亮藍，一整面牆用它會過曝，而且白色的水塔就跳不
   * 出來了。`Utility.test.ts` 比的是**色相與地形水面一致**，不是這個十六進位
   * 值：哪天地形的水改色，那條會要求這裡跟著改。
   */
  water: rgb(0x2a76b8),         // 河藍
