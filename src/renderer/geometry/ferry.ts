...
/** Ferry: an enlarged hull with a triangular bow, a cabin, fore and aft decks, and railings. */
export function buildFerryGeometry(): THREE.BufferGeometry {
  const parts: THREE.BufferGeometry[] = [];

  // The hull, the main lower body.
  const hull = new THREE.BoxGeometry(0.65, 0.05, 0.22);
  hull.translate(0, 0.025, 0);
  setVertexColors(hull, 0.95, 0.95, 0.95);
  parts.push(hull);

  // The bow: a triangular point facing +X.
  const bowGeo = new THREE.BufferGeometry();
  const bowY = 0.025;
  const bowVerts = new Float32Array([
    // Bottom triangle.
    0.325, bowY - 0.02, -0.11,   // aft left, at the hull front
    0.325, bowY - 0.02,  0.11,   // aft right
    0.425, bowY - 0.02,  0.0,    // the point
    // Top triangle.
  ...
  // The cabin, centred slightly forward.
