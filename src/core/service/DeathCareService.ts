...
export class DeathCareService extends RoadCoverageService<Cemetery> {
  ...
  /** Try to assign a pending death to the nearest cemetery with available hearses and capacity. */
  private tryAssignDeath(death: PendingDeath): void {
    ...
    for (const cem of this.facilities) {
      ...
      // All hearses busy — must wait for one to return
      if (cem.inTransit >= DEATH_CARE.HEARSE_COUNT) continue;
