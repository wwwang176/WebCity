...
export class Grid {
  ...
  fillCell(x: number, y: number, out: CellData): void {
    const offset = this.getOffset(x, y);
    const idx = y * this.width + x;
    out.terrainType = this.view.getUint8(offset + CELL_OFFSET.terrainType);
    out.zoneType = this.view.getUint8(offset + CELL_OFFSET.zoneType);
    out.buildingId = this.view.getUint16(offset + CELL_OFFSET.buildingId, true);
    out.roadFlags = this.view.getUint8(offset + CELL_OFFSET.roadFlags);
    out.roadType = this.view.getUint8(offset + CELL_OFFSET.roadType);
    out.trafficDensity = this.view.getUint8(offset + CELL_OFFSET.trafficDensity);
    out.landValue = this.view.getUint8(offset + CELL_OFFSET.landValue);
    out.pollution = this.view.getUint8(offset + CELL_OFFSET.pollution);
    out.noiseLevel = this.view.getUint8(offset + CELL_OFFSET.noiseLevel);
    out.serviceCoverage = this.view.getUint8(offset + CELL_OFFSET.serviceCoverage);
    out.elevation = this.view.getInt8(offset + CELL_OFFSET.elevation);
