...
function makeRoadLookup(cellKeys: string[]): UnifiedRoadLookup {
  ...
  return {
    ...
    // This stub is cast in through `as unknown as UnifiedRoadLookup`, so type checking cannot
    // catch a missing method and one omission is a runtime TypeError. Every rule added to
    // findBuildingAccessPoints needs a counterpart here.
