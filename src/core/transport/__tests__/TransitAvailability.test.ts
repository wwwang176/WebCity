...
describe('findAvailableTransit', () => {
  it('returns empty array when no transit systems exist', () => {
    const result = findAvailableTransit([], { x: 0, y: 0 }, { x: 10, y: 10 }, 5, openFieldReach, WALK_SPEED, WAIT_FACTOR);
  ...
  it('returns empty array when no route has stops near both origin and destination', () => {
    ...
    const result = findAvailableTransit(systems, { x: 0, y: 0 }, { x: 10, y: 10 }, 5, openFieldReach, WALK_SPEED, WAIT_FACTOR);
    expect(result).toEqual([]);
  });
  ...
  it('only considers stops within walk range', () => {
    ...
    const result = findAvailableTransit(systems, { x: 1, y: 1 }, { x: 5, y: 5 }, 2, openFieldReach, WALK_SPEED, WAIT_FACTOR);
  ...
  it('uses segment distances when provided (bus with detour)', () => {
    ...
    const systems: TransitSystemInfo[] = [{
      type: TransportType.BUS,
      speed: 2,
      routes: [{ id: 1, type: TransportType.BUS, stops, vehicles: 1, frequency: 0, operatingCost: 100 }],
    ...
    const result = findAvailableTransit(systems, { x: 0, y: 0 }, { x: 10, y: 0 }, 5, openFieldReach, WALK_SPEED, WAIT_FACTOR);
    expect(result).toHaveLength(1);
  ...
  it('falls back to euclidean stop-to-stop distance when no segment distances', () => {
    ...
    const systems: TransitSystemInfo[] = [{
      type: TransportType.METRO,
      speed: 3,
      routes: [{ id: 1, type: TransportType.METRO, stops, vehicles: 1, frequency: 0, operatingCost: 300 }],
    }];
    const result = findAvailableTransit(systems, { x: 0, y: 0 }, { x: 3, y: 4 }, 5, openFieldReach, WALK_SPEED, WAIT_FACTOR);
    expect(result).toHaveLength(1);
  ...
  it('handles circular route forward traversal correctly', () => {
    ...
    const result = findAvailableTransit(systems, { x: 5, y: 5 }, { x: 0, y: 0 }, 5, openFieldReach, WALK_SPEED, WAIT_FACTOR);
    expect(result).toHaveLength(1);
  ...
  it('still charges the walk when origin and dest map to the same stop', () => {
    ...
    const result = findAvailableTransit(systems, { x: 0, y: 0 }, { x: 0, y: 1 }, 5, openFieldReach, WALK_SPEED, WAIT_FACTOR);
    expect(result).toHaveLength(1);
  ...
  it('ferry uses segment distances from water path cache', () => {
    ...
    const systems: TransitSystemInfo[] = [{
      ...
      speed: 0.375,
      routes: [{ id: 1, type: TransportType.FERRY, stops, vehicles: 1, frequency: 0, operatingCost: 200 }],
      // Water path distance = 15 (longer than euclidean ~11.3)
      getSegmentDistances: (routeId: number) => routeId === 1 ? [15, 15] : null,
    }];
    const result = findAvailableTransit(systems, { x: 0, y: 0 }, { x: 8, y: 8 }, 5, openFieldReach, WALK_SPEED, WAIT_FACTOR);
    expect(result).toHaveLength(1);
  ...
  it('adding vehicles increases route capacity (allows more riders)', () => {
    ...
    expect(findAvailableTransit([sys3], { x: 0, y: 0 }, { x: 10, y: 10 }, 5, openFieldReach, WALK_SPEED, WAIT_FACTOR)).toEqual([]);
    ...
    expect(findAvailableTransit([sys5], { x: 0, y: 0 }, { x: 10, y: 10 }, 5, openFieldReach, WALK_SPEED, WAIT_FACTOR)).toHaveLength(1);
  });
  ...
  it('filters only full routes, keeps routes with capacity from same system', () => {
    ...
    const systems: TransitSystemInfo[] = [{
      ...
      vehicleCapacity: 50,
      routes: [
        { id: 1, type: TransportType.BUS, stops: stopsA, vehicles: 2, frequency: 0, operatingCost: 100 },
        { id: 2, type: TransportType.BUS, stops: stopsB, vehicles: 2, frequency: 0, operatingCost: 100 },
      ],
    }];
    const result = findAvailableTransit(systems, { x: 0, y: 0 }, { x: 10, y: 10 }, 5, openFieldReach, WALK_SPEED, WAIT_FACTOR);
    expect(result).toHaveLength(1);
...
describe('getRouteDailyRiders', () => {
  it('sums dailyRiders across all stops', () => {
    ...
    const route = { id: 1, type: TransportType.BUS, stops, vehicles: 1, frequency: 0, operatingCost: 100 };
