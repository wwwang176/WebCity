...
- [x] 2LINE 直路段產生正確數量的 ConnectionPoint（每方向 1 lane × 2 點 = 4 點/cell）
- [x] 4LINE 直路段產生正確數量（每方向 2 lanes × 2 點 = 8 點/cell）
- [x] 同方向相鄰 lane 之間有 lane_change 類型 LaneEdge
- [x] 2LINE→4LINE 銜接：lane0 正確映射，lane1 為額外車道
- [x] 十字路口（4-way + 4LINE）產生 ≤ 24 條 turn 類型 LaneEdge
- [x] T 字路口只產生通往 2 個出口方向的 turn 邊
- [x] 建路/拆路後 LaneGraph 局部更新正確，不影響無關區域

#### 單元驗收 — Phase B: Bezier 曲線 ✅

- [x] 90° 轉彎 Bezier 控制點生成平滑弧線（無尖角）
- [x] 弧長參數化 LUT 等距采樣誤差 < 1%
- [x] Bezier 曲線任意 t 取得的 position 和 tangent 數值正確
...
- [x] Cell-level A* + Lane-level refinement 兩階段產生完整 LaneEdge 路徑
- [ ] 右轉車輛提前靠右車道、左轉提前靠左
- [ ] 換道代價高於直行，避免不必要換道
- [ ] 無法完成所需換道時選擇替代路線
...
- [x] 車輛在十字路口轉彎平滑（Bezier 曲線），無 90° 瞬轉
- [x] 換道動畫自然（車輛斜向滑動至相鄰車道）
- [x] 車頭朝向與行駛曲線切線方向一致
- [ ] 不同寬度道路銜接處車輛過渡自然
...
- [x] 100 輛車在混合道路寬度（2LINE/4LINE/6LINE）路網中行駛，無車輛穿牆或卡住（BUG-048 修復後驗證通過）
- [x] 十字路口多方向車流交會無碰撞（不同 turn 邊的車輛各走各的弧線）— 4x4 路網 16 個十字路口視覺驗證通過
- [x] 大量車輛在十字路口排隊等紅燈 → 綠燈後依序通過、正確轉彎 — trafficLight phase 切換 + canAdvance 阻擋驗證通過
- [x] 效能：63 輛 edge 車輛 + Lane Graph → 0.74ms/tick（遠低於 50ms 上限）
