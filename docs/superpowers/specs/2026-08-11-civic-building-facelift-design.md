...
### 4.6 代表色（使用者於 2026-08-11 追加）

舊版每一種公共建築都有自己的顏色（警局靛藍 `0x3f51b5`、消防局紅
`0xd32f2f`、醫院白、公園綠……）。那是玩家辨認它們的**主要**訊號：等角視角
下，剪影要縮到很近才分得出「L 形加瞭望塔」與「雙翼加連廊」，顏色卻一眼
就看得出來。

改走 `BUILDING_FRAG` 之後這個訊號會**消失**，而且是靜默的：牆色來自
`vBldgColor`，那個 varying 在 `USE_INSTANCING_COLOR` 之外的分支寫死
`vec3(0.7)`。公共建築在遊戲裡是 `THREE.Group` 底下的普通 `Mesh`、在展示區
也是普通 `Mesh` —— 兩者都沒有 `instanceColor`，所以會**全部變成同一片灰**。

修法是新增逐幾何的頂點屬性 `aBldgColor`：

```glsl
attribute vec3 aBldgColor;
...
#ifdef USE_INSTANCING_COLOR
  vBldgColor = instanceColor;   // 分區建築，遊戲路徑不變
#else
  vBldgColor = aBldgColor;      // 公共建築與展示區
#endif
```

顏色表在 `civic/colors.ts`，數值直接沿用舊版的十六進位色。唯一改掉的是
垃圾場 —— 它的舊值與小學一模一樣（都是 `0x795548`），在舊版看不出來是因為
兩者造型差很多，改走同一套立面之後「褐色的方塊」會真的分不出來。

**逐量體覆寫。** `CivicVolume.color` 蓋過 plan 的底色，給醫院的紅十字、
大學的金頂、車站的識別帶用。一棟只有一個顏色的話，這些重點只能跟牆同色
—— 而它們正是「一眼認出這是醫院」的東西。實作上與 `tagPart` 完全同一個
機制：合併前逐量體寫進頂點屬性。

**一個必須守住的交互作用：** `stampInstanceValues` 會補齊 shader 宣告的每個
屬性，而 `assembleCivic` 已經逐量體寫過 `aBldgColor`。前者若無條件重寫整份，
覆寫會被抹平 —— 畫面上只表現為「紅十字不見了」。所以它遇到既有的
`aBldgColor` 要跳過。
...
**物件與綠化的額度刻意開得比分區建築寬**（使用者於 2026-08-11 指定）。
分區建築鋪滿整張地圖，一棟多十個三角形要乘上幾千棟；公共建築一座城市裡
也就幾十棟，單棟多花的成本幾乎量不到。所以「這裡多種幾棵樹」在公共建築
是划算的，而在住宅區同樣的想法會直接打爆預算 —— 兩者的取捨本來就不同，
用同一組數字才是錯的。對照：分區建築的矮物件上限是每棟 320
（`TRIANGLE_BUDGET.PROP`）。

這些數字仍然在批 1 做完之後要用實測回頭校準，**不是推導出來的**。
（警局實測：量體 92、貼片 20、矮物件 304、懸挑 12，總計 428。）
