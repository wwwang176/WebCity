...
整圈時間 = 路線總長 / 現在的實際速度
班距     = 整圈時間 / 車輛數
每日運能 = 車輛數 × 每車座位 × (TRANSIT_SERVICE_TICKS_PER_DAY / 整圈時間)
載重率   = 一整天的搭乘人次 / 每日運能
等車時間 = 班距 × (0.5 + max(0, 載重率 - 1))
```
...
`TRANSIT_SERVICE_TICKS_PER_DAY = 480` **不是**日曆上的一天（`ticksPerDay = 24`）。

車速是挑出來讓畫面看起來像公車的，不是從物理推出來的。拿日曆去除動畫等於把兩個
時鐘當成同一個 —— 一條 282 格的公車路線一圈要 141 tick，日曆上一天只跑 0.17 圈，
50 座的車一天運能剩 **8.5 人次**，任何路線都得買三百台車（BUG-344）。

480 是**平衡旋鈕**，選它而不是 960 的理由（960 之下捷運永遠吃不滿，擁擠模型在捷運上
等於不存在）寫在 `RouteLoad.ts`。代價是明知道的：畫面上那台車一天還是只跑 0.17 圈。

### 等車沒有上限，也沒有懸崖

擠不上這班的機率是 `q`，平均要多等 `q / (1 - q)` 班；以 `q = 1 - 1/載重` 代入，
化簡剛好是 **載重 - 1 個班距**。基本等待是**半個**班距（乘客隨機到站），多等的是
**整班** —— 兩者單位不同，所以是相加不是相乘。

舊模型是三個手挑的數字加一道懸崖：載重 0.8 起線性爬到 4 倍，1.5 就把整條路線從這個人
的選項裡拿掉。那道懸崖自己造出一個極限環 —— 加車讓載重衝過 1.5，全部人被踢出去，
載重掉回來，人又回來（BUG-347）。現在**沒有拒載線**：等待自己會發散，而一條要等十班
的路線在比大小的時候自己就輸了。

### 載重比的是一整天

「一整天的搭乘人次」取「**昨天的實數**」與「跨日平滑值」的較大者。讀今天的累計會讓
載重每個遊戲日鋸齒一次（玩家存檔實測 5.56 ~ 47.34）—— 運能的單位是一整天，而累計量
每天歸零，於是每天早上每條路線看起來都是空的，所有人都選它，傍晚才爆掉（BUG-348）。
代價是新路線第一天看起來是空的：一天的資料要滿一天才有。

面板把載重分成四段（`comfortable` < 1 / `crowded` ≥ 1 / `overloaded` ≥ 1.5 /
`hopeless` ≥ 3）。**那是顯示用的分界，不是模擬門檻** —— 模擬只是讓它越來越慢。
...
班距與載重由 `refreshRouteService()` 每 tick 重算（`MultiModalRouter.ts`）。扁平化的
`FlatRoute` 帶的是**來源路線的參照**與一個取速度的函式，不是複製下來的數值 —— 複製
一份的話，加車與塞車都不會反映出來，正是[路線結構](#路線結構)那一節警告的同一個錯
（BUG-343）。
...
壅塞率是**這條路線自己沿途的平均**（`congestionOn(routeId)`），不是全城平均。用全城
平均的話，塞在市中心的那條線跟郊區空蕩蕩的那條線一樣快，玩家改不動任何東西（BUG-339）。

這個速度不只是畫面上的車跑得慢。`getSpeedOn(routeId)` 回的是**現在**的實際速度，
運具選擇那一關讀的也是它 —— 開車那一側滿滿地計入壅塞（`距離 × (1 + 壅塞)`），公車
那一側如果讀設定值，塞車的城市裡公車會看起來不合理地好：路上的車全部慢下來，只有
公車照跑（BUG-345）。塞車因此讓公車班距變長、運能變小、載重變高。

其他大眾運輸（地鐵/鐵路/渡輪）不受壅塞影響 —— 那正是玩家蓋捷運的理由。
...
**沒有「滿載就不能搭」這回事。** 載重高只是讓等車變久（見[班距與擁擠](#班距與擁擠)），
一條要等十班的路線在比大小的時候自己就輸了。

舊版在 `dailyRiders 總和 ≥ 車輛數 × 單車容量` 時把整條路線從選項裡拿掉 —— 一個累計量
比一個瞬間量，兩台公車一天載到第 100 人次就算滿（BUG-344）；而那道不連續自己造出震盪
（BUG-347）。
