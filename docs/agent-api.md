...
```json
{
  ...
  "cells": [
    {"x":35,"y":9,  "cost":0,   "ratio":0.00, "load":0.48, "facilityId":"police_1",
     "severity":0.00, "tier":0, "color":"#00e676"},
    {"x":34,"y":24, "cost":234, "ratio":0.43, "load":1.69, "facilityId":"police_3",
     "severity":0.69, "tier":6, "color":"#ffc944"},
    {"x":30,"y":56, "cost":540, "ratio":1.00, "load":0.20, "facilityId":"police_1",
     "severity":1.00, "tier":9, "color":"#ff5252"}
  ],
  "sources": [{"x":22,"y":28}, ...]
}
```

**顏色吃的是 `severity`，不是 `ratio`。** 兩件事會讓一格變紅:

| | 欄位 | 玩家要做的事 |
|---|---|---|
| 太遠 | `ratio`（1 = 剛好在覆蓋邊界） | 蓋一座**近**的 |
| 太滿 | `load`（1 = 剛好滿，2 = 需求兩倍） | 蓋一座**分流**的 |

`severity` 是兩者取比較糟的一個。中間那一列就是重點:設施只有 0.43 遠 ——
以前那一格是綠的 —— 但服務它的那座爆到 1.69 倍，所以它是橘的（BUG-362）。

`facilityId` 說的是**哪一座在管這一格**（沿馬路走過來最便宜的那一座）。
`sources` 說「城裡有這些設施」，`facilityId` 說「就是這一座」—— 一片紅色要去動
哪一棟，看它。

`load` 不夾在 1:2.0 跟 11.86 是兩件事，後者是那份存檔裡的高中。
`-1` 代表那個服務沒有逐格負載的概念。
