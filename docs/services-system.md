...
工業建築生產貨物，沿道路 BFS 運送到商業建築。距離工業區近的商業優先取貨。
...
從每棟工業建築出發，沿道路 BFS 擴散：
- 每經過一棟商業建築，扣除其消耗量（budget-drain）
- 預算耗盡後停止 → 遠處商業可能缺貨
- 無道路連接的商業永遠缺貨

### 缺貨效果

商業建築未被 BFS 到達（`isSupplied = false`）時：
- 廢棄壓力 +6（與無供水同級）
- 營收減半（× 0.5）
- 商業 RCI 需求降低（`shortageRatio × 10`）

注意：城市中沒有工業建築時不會產生缺貨效果。

### 過剩效果

工業生產超過商業消耗時，多餘貨物存入 `cargoStorage`（上限 200）：
- 工業廢棄壓力 = `surplusRatio × 6`
- 工業營收減少 = `× (1 - surplusRatio × 0.5)`
- 工業 RCI 需求降低（`surplusRatio × 10`）

注意：城市中沒有商業建築時不會產生過剩效果。

### 貿易系統（Import / Export）

當城市有鐵路車站（鐵軌連接到地圖邊緣）或機場時，可進行進出口貿易：

**前提條件**：
- 鐵路：車站必須透過鐵軌 BFS 連通到地圖邊緣（閉環鐵路不算）
- 機場：有機場建築即可

**吞吐量**：每座外部車站 50/tick，機場按大小 20/100/300

**進口**（缺貨時）：商業從外部進口，收入 ×0.7，無廢棄壓力
**出口**（過剩時）：工業出口到外部，降低過剩壓力，收入 ×0.5
