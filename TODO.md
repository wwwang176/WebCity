...
- [ ] **第三批條例**（育兒補貼、義務教育、免費診所、禁菸令、壅塞費）。要先決定每個
  槓桿接在哪一行，那是設計問題不是實作問題，另開一份計畫。
...
### 分區筆刷（BUG-297 之後）
- [ ] **重複的入口**。工具列上「作用中分區」那一格與 Districts 那顆開的是同一個
  面板。留著 Districts 是因為手上沒有分區時前者不能按，但兩顆做同一件事。
- [ ] **空分區清不掉**。`DistrictManager` 沒有 `deleteDistrict`。New 改成放掉選取
  之後不會再產生空分區，但用扣除把一區挖光還是會留下一個 —— 它身上的條例設定要
  留著（見 DistrictPaint 的測試），所以不能自動刪，需要一顆明確的刪除鈕。
- [ ] **單格搶別區的格子做不到**。點一下別區的格子現在是「切過去編輯」，所以要把
  別區的單獨一格併進來只能拖一個更大的矩形再扣掉多的。取捨是刻意的（見 BUG-297），
  真的有人需要再想。
