...
- [ ] **BUG-267：pathfinding worker 在真實大小的城市裡一條路徑都找不到**（見 BUGS.md）。
  60×60、4 800 邊的城市，worker 對每一組起迄都回傳空的；主執行緒同一組找得到 131 段。
  24×24 的測試城市沒事，所以既有測試全綠。要查的是 SAB 上那份圖與 `PooledAStar` 的
  展開上限（主執行緒的 `laneAStar` 是 8 000 步）。修好之後 BUG-265 的補完會從
  每 tick 2 條變成 32 條，全城補完從約 1.5 分鐘縮到幾秒。
- [ ] **補完全城通勤路線要約 1.5 分鐘**（1x 速度，2 146 人的存檔）。這是 BUG-267 沒修
  之前的數字 —— 主執行緒每 tick 只敢算 2 條（一次約 16 毫秒）。期間預測車流與噪音
  汙染會由低往上爬，不是穩定值。
