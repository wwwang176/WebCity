...
- [x] **公車的壅塞改成逐路線算**（BUG-339，2026-08-20）— `congestionLevel` 留作退路，
  ...
  速度倍率 0.895 → 0.810。
- [ ] **公車的「乘車時間估計」完全不含壅塞** — `getTransitSystemInfos()` 傳的是
  `system.getSpeed()`，也就是 `config.speed` 的**原始值**;`findAvailableTransit` 用它算
  `rideDistance / speed`。所以運具選擇那一關:**開車滿滿地計入壅塞，公車完全不計** ——
  塞車的城市裡公車看起來不合理地好。逐路線的壅塞（BUG-339）已經有了，接上去只差把
  `speed` 改成逐路線。
  **但這會動平衡**（BUG-324 修完時開車佔比從 47% 升到 57%，那條線很敏感），而玩家
  存檔目前 `DRIVE 8801 / RAIL 40`、**公車零人**，量不出影響。要做的話得先造一座真的
  有人搭公車的城市來量。
