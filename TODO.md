...
- [ ] **`refreshCommuteStats` 的逐人估算要分片** — 拆開量過:10 萬人時估算 114ms、
  加總 13.5ms（換掉排序之後），估算佔 **89%**。做法是每個 tick 算 1/N 的市民、把**每個
  人的值存起來**，統計從存下來的全部值算 —— 不是抽樣。抽樣被否決，理由有二:
  (1) `chargedDriversByDistrict` 直接決定壅塞費**收入**，抽樣等於讓玩家的錢跟著抽到誰而抖;
  (2) 固定抽 k 人是**系統性偏差**不是隨機誤差 —— 那棟樓永遠顯示錯的數字，不會自己修正。
  片數用 `clamp(60, ceil(人口/2100), 72)`:12.6 萬人以下回傳 60，**每位市民的更新頻率
  與現在完全相同**（60 tick = 2.5 遊戲日 = 速度 1 的 15 秒、速度 10 的 1.5 秒）。
