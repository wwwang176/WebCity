...
**測試**：4352 → 4415。新增 `PropBands`、`Decals`、`OverheadProps`、`GroundLayers` 四支。

**階段 2B-2 途中量到的缺陷**：BUG-224（分區建築放在**路面**高度 0.05 而不是地面
高度，連同十九種基礎設施一起浮空 0.6 m）。

### 階段 2C 待辦（尚未排計畫）

- [ ] 參數化量體生成器：九種原型，每桶 8 個變體，重複率 23.2% → 5% 以下
- [ ] 等級的豪華階梯：量體／材質／零件三項（周邊已在 2B 完成）
- [ ] 色盤與 `aSeed.z` 加上等級維度
- [ ] 工業的等級階梯改用煙囪／筒倉／管架／貨櫃，不再靠高度
