...
- [ ] **`JobRelocationSlicer` 該照 BUG-331 的方式重做**（見 BUGS.md）— 它仍是「一份名單
  攤上百個 tick」的舊形狀。墓碑已經接上去（死人不會再被換工作），但候選工作地中途被拆
  還沒擋。治本不是再補一個檢查，而是照換房子的方式改成**單 tick 分批**:每次只叫一部分
  人來，快照當場用完丟掉。補丁補了三輪還在冒新的，那條路已經證明走不通。
