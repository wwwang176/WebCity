...
- [ ] **BUG-328 每 0.25 秒卡 40ms** — `spawnCommuteVehicles` 每 tick 對 1,238 位取樣到的
  ...
  **不能單純把生成點檢查提前** —— 這個迴圈同時在累計大眾運輸的 `dailyRiders` 與行人
  `pendingTrips`，先擋掉非開車的人會讓搭乘數垮掉。要嘛把兩件事拆開，要嘛快取
  逐 OD 的運具決策（流量圖 60 tick 才換一次，決策在那之間是固定的）。
- [ ] **BUG-329 讀檔後前 11 秒** — `advanceCommuteFill` 的預算用完之後用 `continue`
  而不是 `break`，整份 12,351 人的名單每個 tick 都掃完。前 11 秒吃掉 46–66% 的
  `update()` 時間，之後掉到 2%。順帶:每個 tick 都從名單開頭重掃，排後面的市民
  要等前面的人全部 settled 才輪得到 —— 改成游標式續掃可以一起解決。
