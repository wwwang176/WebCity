...
- [ ] Task 4：`waterDemand` 槓桿 + 節水法規。**乘數必須同時作用在 `totalDemand`
  與供水 BFS 的逐格需求**，只降帳面數字的話缺水的建築一棟也不會恢復供水，而
  「`getDemand()` 變小」這種測試守不住那個壞實作。
- [ ] Task 5：`sewageLoad` 槓桿 + 汙水處理標準
- [ ] Task 6：`industrialPollution` 槓桿 + 工業排放管制
- [ ] Task 7：16 條條例的 UI 分類。測試必須真的碰到兩個 modal —— 只驗分組函式的
  純資料輸出的話，modal 繼續用舊清單也會全綠。另外 `DistrictModal` 目前刻意列出
  「現在提供的 ∪ 這一區已經有的」，直接換成分類清單會讓舊存檔裡已下架的條例消失，
  玩家就再也關不掉它。
- [ ] Task 8：文件與收尾

### 條例目錄的設計缺口（Codex 審查發現，尚未處理）
- [ ] **賭場 / 夜間經濟 / 宵禁沒有互斥**。目錄裡寫著「同一塊地只能選一邊」，實作上
  三條可以同時開。賭場疊夜間經濟是商業收入 +68.75%、再疊觀光是 ×2.025。要做的是
  一個 exclusive group:開一條就把同組其他條關掉，而且存檔還原也要重跑這條規則。
- [ ] **代價可能落空**。宵禁套在沒有商業的住宅區只有好處沒有代價;歷史保存套在
  純工業區同理。靜態的 `PolicyTradeoff` 檢查只看效果物件裡有沒有代價欄位，看不到
  那個代價在這座城市裡有沒有承擔者。
- [ ] **逐級方向沒有不變量**。多數測試只驗某一級;把夜間經濟第二級的收入改成負的、
  或把宵禁第二級的犯罪改成正的，現在都不會被抓到。
- [ ] **範圍測試是同語反覆**。分類與一致性測試都從 `POLICY_SCOPE` 推導預期再驗
  `POLICY_SCOPE`。要一份獨立的設計 oracle 明確寫死每一條的範圍。
- [ ] **工業排放管制的範圍值得重想**。汙染源是逐格的工業格，好處與代價也都落在
  工業格 ——「管哪一區的工廠」在機制上是有意義的決策。現在放全城是把它當法規
  一致性處理，那是產品決策，不是機制推導出來的。
