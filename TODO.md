...
- [x] 號誌桿原本是 `0x333333`，貼在黑色柏油上幾乎看不見 —— 貼近看時四支只有
  ...
  而共用同一個常數就不會有第二份可以漂移。
- [ ] **燈泡縮小之後，正常遊玩的縮放下讀不到號誌了**。燈頭從 0.036 縮到
  `STREET_LAMP_BULB_RADIUS`（0.018，路燈燈泡的一半直徑）之後，貼近看很合比例，
  但視野拉到 10 格高就看不出紅綠。號誌在那個距離本來就只是一個彩色點，而現在
  那個點只剩四分之一的面積。要嘛燈泡回到中間值，要嘛遠景另外給號誌一個
  「只有顏色、不縮小」的表示法。
