...
- [x] **TEST**: 老人死亡 → 需墓園/火葬場處理
- [x] **TEST**: 處理不及 → 產生「死亡波」問題（大量骨灰車）
- [x] 實作 DeathCare 模組

### 10.11 服務車輛調度

- [x] **TEST**: 服務車輛（消防/救護/垃圾/殯葬）使用路網出勤
- [x] **TEST**: 出勤路徑受交通壅塞影響
- [x] **TEST**: 可將服務設施指派到特定 District
- [x] 實作 ServiceDispatch 模組
...
- [x] 建立 simulation.worker.ts
- [x] Worker 內運行 SimulationLoop
- [x] 寫入結果到 SharedArrayBuffer
- [x] 與主線程的 postMessage 通訊
