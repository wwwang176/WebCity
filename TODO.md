...
- [ ] **車輛處理順序可能根本不需要排序** — 突變驗證發現把排序**反過來**（後車先處理）
  全套測試沒有一個紅。原因看起來是所有查詢讀的都是幀開頭的快照（`edgeIndex` 連
  `queueing` 都是那時候抓的），所以處理順序影響不到任何判斷。程式裡的註解說「後車
  這一幀讀到的是剛算好的值」，那句話可能已經過時。若屬實，每幀 20.6ms 的排序可以整個
  拿掉。**要先查清楚再動** —— 也可能是缺一個測試而不是缺一個需求。
