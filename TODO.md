...
- [ ] **`SHADOW_BIAS` 的世界距離沒有被算過。** 它是深度空間的值，實際位移要
  乘上陰影相機的深度範圍：`0.0005 × (SHADOW_FAR - SHADOW_NEAR)` =
  `0.0005 × 199 ≈ 0.1 單位 ≈ 1.2 公尺` 沿光軸。若推導成立，它比已修好的
  `normalBias`（21 公分）更大。根因是 `SHADOW_NEAR: 1 / SHADOW_FAR: 200`
  把深度範圍開得遠比需要的寬 —— 光源距焦點只有 √(50²+80²+50²) ≈ 107 單位，
  建築最高 48 m = 4 單位。收緊 near/far 會直接把 bias 的世界距離按比例縮小，
  而且不必動 bias 本身。
  **需要實機確認**：猜錯的方向是整片地面長出 shadow acne。
