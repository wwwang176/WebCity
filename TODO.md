...
### 階段 2B 待辦（計畫：`docs/superpowers/plans/2026-08-09-building-model-variety-phase-2b.md`）

- [ ] BUG-222：基地縮放上限要把抖動與單邊外凸算進去；鋪滿基地的分區取消寬深抖動
- [ ] 實例桶機制抽成 `InstancedLayer`，讓地面物件層不必複製一份
- [ ] `groundProps.ts`：住宅低密度 L1/L2/L3 的庭院組合，依真實尺寸生成
- [ ] 地面物件獨立圖層，**不吃建築的 Y 縮放** — BUG-219
- [ ] 建築幾何不再含 `PART_FOLIAGE`
- [ ] 展示區顯示地面物件層
