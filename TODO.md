...
- [ ] **BUG-109 治本：把高架層序列化進 workplace-distance worker 的緩衝區。**
  現在只要有一格高架道路，整座城市的距離快取就停用（連算都不會算），
  341 人的城市每輪要 1474 ms 的 Dijkstra。**這一項要先寫 spec** —— 要決定
  稀疏的高架層怎麼裝進平面的 byte 緩衝、匝道怎麼接，而且有一條硬約束：
  worker 算出來的距離必須與主執行緒的同步版本逐格一致。
- [ ] **量一下快取實際有多少比例的時間是 ready。** `invalidatedDuringBuild`
  的路徑意味著：就算城市裡完全沒有高架，只要建築一直在變，快取也可能永遠
  ready 不了（作廢 → 開始算 → 算到一半又被作廢 → 丟掉 → 重來）。
  **如果真是這樣，治本的效益會比預期小很多，得先知道。**
