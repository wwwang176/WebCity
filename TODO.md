...
- [ ] **壅塞費收上來的錢沒有進市庫**。目前只收門架與稽查的維運費，而現實裡壅塞費
  是一條收入。整條計費管線只會扣錢:`policyCost` 的結果進 `calculateTotalExpenses`，
  而「每一級的費用要更貴」是有測試釘住的不變量 —— 負的單價會把那個不變量整個翻
  過來。要做的話得先決定條例可不可以有收入面，那會動到帳本的呈現（一筆負支出在
  面板上讀起來像什麼）。
